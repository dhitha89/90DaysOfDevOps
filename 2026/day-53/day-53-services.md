...
What IP does `nslookup` return? Does it match the CLUSTER-IP from `kubectl get services`?
...
web-app-clusterip   ClusterIP   10.96.15.253   <none>        80/TCP    54m
```
The Cluster-IP match
...
(On WSL2, Docker containers sit on a private virtual network (172.22.x.x), so ports aren't exposed to your host's localhost unless explicitly mapped )
Since I use WSL we access the url using 172.22.0.5
curl http://172.22.0.5:30080

Can you see the Nginx welcome page from your browser or terminal using the NodePort?
...
Yes the load balance has the Nodeport IP and Cluster IP
